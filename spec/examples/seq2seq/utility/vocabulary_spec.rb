...
describe Examples::Seq2seq::Utility::Vocabulary do
  ...
  let(:double_quote) {Examples::Seq2seq::Utility::Vocabulary::DQ}
  ...
  context '#word_to_id' do
    it '辞書に存在する単語に対して正しいidが返ってくる' do
      expect(vocabulary.word_to_id('ball')).to eq 3
      expect(vocabulary.word_to_id('paper')).to eq 4
    ...
    it 'クラス定数で定義した単語に対して正しいidが返ってくる' do
      ...
      expect(vocabulary.word_to_id(double_quote[:word])).to eq double_quote[:id]
    end
  end

  context '#id_to_word' do
    it '辞書に存在するidに対して正しい単語が返ってくる' do
      expect(vocabulary.id_to_word(3)).to eq 'ball'
      expect(vocabulary.id_to_word(4)).to eq 'paper'
    ...
    it 'クラス定数で定義したidに対して正しい単語が返ってくる' do
      ...
      expect(vocabulary.id_to_word(double_quote[:id])).to eq double_quote[:word]
    end
  ...
  context '#size' do
    it '辞書の語数 + 3 が返ってくること' do
      expect(vocabulary.size).to eq dictionary.size + 3
    end
  end
  ...
  context '辞書内の単語が重複している場合' do
    ...
    context '#word_to_id' do
      it '辞書に存在する単語に対して正しいidが返ってくる' do
        expect(vocabulary.word_to_id('ball')).to eq 3
        expect(vocabulary.word_to_id('paper')).to eq 4
        expect(vocabulary.word_to_id('cup')).to eq 6
      end
    end
    ...
    context '#id_to_word' do
      it '辞書に存在するidに対して正しい単語が返ってくる' do
        expect(vocabulary.id_to_word(3)).to eq 'ball'
        expect(vocabulary.id_to_word(4)).to eq 'paper'
        expect(vocabulary.id_to_word(5)).to eq unknown_word[:word]
        expect(vocabulary.id_to_word(6)).to eq 'cup'
      end
    end

    context '#size' do
      it '辞書の語数 + 3 が返ってくること' do
        expect(vocabulary.size).to eq dictionary.uniq.size + 3
      end
    end
  end
end
